...
export type ContactLeadVariant = "enterprise" | "b2c";

type Props = {
  open: boolean;
  onOpenChange: (o: boolean) => void;
  variant?: ContactLeadVariant;
  source?: string;
};
...
> = {
  enterprise: {
    ...
    description:
      "Kurumsal çözümler için formu doldurun; size e-posta ile dönelim.",
    nameLabel: "Ad / unvan",
    namePlaceholder: "Örn. ACME A.Ş.",
    emailPlaceholder: "kurumsal@ornek.com",
    messagePlaceholder:
      "Ekip büyüklüğü, entegrasyon ihtiyacı, tahmini hacim…",
    defaultSource: "enterprise",
  },
  b2c: {
    title: "Teklif alın",
    description:
      "Bireysel kullanım, paket veya özel destek için yazın. Kısa sürede dönüş yaparız.",
    nameLabel: "Adınız",
    namePlaceholder: "Ad Soyad",
    emailPlaceholder: "ornek@email.com",
    messagePlaceholder:
      "Ne için destek istiyorsunuz? (kira, iş sözleşmesi, dilekçe, asistan…)",
    defaultSource: "pricing_b2c",
  },
};
...
export function ContactLeadDialog({
  ...
  const leadSource = source ?? copy.defaultSource;
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="grid gap-3 py-2">
              ...
              <div className="grid gap-1.5">
                <Label htmlFor={`lead-name-${variant}`} className="font-medium">
                  {copy.nameLabel}
                </Label>
                <Input
              ...
              <div className="grid gap-1.5">
                <Label htmlFor={`lead-email-${variant}`} className="font-medium">
                  E-posta
                </Label>
                <Input
              ...
              <div className="grid gap-1.5">
                <Label htmlFor={`lead-msg-${variant}`} className="font-medium">
                  Mesaj
                </Label>
                <textarea
                  id={`lead-msg-${variant}`}
...
/** Geriye uyum — kurumsal iletişim */
export function EnterpriseContactDialog(props: {
  open: boolean;
  onOpenChange: (o: boolean) => void;
  source?: string;
}) {
  return (
    <ContactLeadDialog {...props} variant="enterprise" />
  );
}
